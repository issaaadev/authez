...
module.exports = class extends EventEmitter {
  ...
  async manageJoin(obj, message, ratelimit) {
    let guild = this.client.guilds.cache.get(obj.guild_id);
    let mbr = obj.amount;
    ...
    let embedss = new this.discord.MessageEmbed()
    embedss.setTitle("__Successfully Pulled Members__")
    embedss.setColor("#36393e")
    embedss.setDescription(`Pulled: ${count}\nMissed: ${obj.amount - count}`)
  ...
  async clean(message) {
    ...
    message.channel.send({
      embed: {
        title: "Cleaned Authorizations",
        description: `**Removed ${count} Authorizations**`,
        color: "GREEN",
  ...
  async refreshTokens(message) {
    ...
    message.channel.send({
      embed: {
        title: "Refreshed Authorizations",
        description: `Refreshed: ${count}`,
        color: "BLUE"
      }
    });
  };
